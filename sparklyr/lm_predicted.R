...
# predicted_path <- "hdfs:///user/dsg/iris/lm/" 
# predicted_name <- "predicted.csv" 




dsg_lm_predict<- function(testpath,modelpath,modelname,predicted_path,predicted_name){
  ...
  write.csv(predicted,predicted_name)
  
  hadoop_cmd <- "/opt/cloudera/parcels/CDH/bin/hadoop"
  system2(hadoop_cmd, paste("fs -put",predicted_name,predicted_path,sep = " "))
